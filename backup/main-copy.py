...
while cap.isOpened():
    ...
    if success:
        ...
        cv2.putText(frame, f"In: {in_counter}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(frame, f"Out: {out_counter}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        if results and hasattr(results[0].boxes, 'id') and results[0].boxes.id is not None:
            ...
            frame = annotated_frame  # Overwrite the frame with the annotated version
